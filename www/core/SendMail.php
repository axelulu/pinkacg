...
class SendMail
{
    private static $MailModelMeta = array();
    public function SendMsg($email,$RegCode,$Model){
        $site_title = SiteMeta::GetSiteMeta('site_name') ?? '';
        $mail = new PHPMailer(true);
        if($smtp_meta = SiteMeta::GetSiteMeta('smtp_meta')){
            $smtp_meta = json_decode($smtp_meta);
        }else{
            exit();
        }
        try {
            ...
            $mail->isSMTP();                             // 使用SMTP
            $mail->Host = $smtp_meta->smtp_host;                // SMTP服务器
            $mail->SMTPAuth = true;                      // 允许 SMTP 认证
            $mail->Username = $smtp_meta->smtp_username;                // SMTP 用户名  即邮箱的用户名
            $mail->Password = $smtp_meta->smtp_pass;             // SMTP 密码  部分邮箱是授权码(例如163邮箱)
            $mail->SMTPSecure = $smtp_meta->smtp_secure;                    // 允许 TLS 或者ssl协议
            $mail->Port = $smtp_meta->smtp_port;                            // 服务器端口 25 或者465 具体要看邮箱服务器支持

            $mail->setFrom($smtp_meta->smtp_username, $site_title);  //发件人
            $mail->addAddress($email, $site_title);  // 收件人
            //$mail->addAddress('ellen@example.com');  // 可添加多个收件人
            $mail->addReplyTo($smtp_meta->smtp_username, $site_title); //回复的时候回复给哪个邮箱 建议和发件人一致
